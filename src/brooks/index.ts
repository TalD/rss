...
import previousItems from '@src/brooks/previousItems'

// Create an RSS feed
function createFeed(items: BookItem[]) {
  const feed = new RSS({
    title: feedDetails.brooks.title,
    description: feedDetails.brooks.description,
    feed_url: '',
    site_url: '',
    language: 'en'
  })
...
async function scrapeData() {
  try {
    const { data } = await axios.get(`${feedDetails.brooks.url}/en_us/mens/shoes/`)
    const $ = cheerio.load(data)
    const items: BookItem[] = $('.o-products-grid__item').map((index, element) => {
      const $element = $(element)
      const href = $element.find('a').attr('href')
      const title = $element.find('.a-type-h5').text()
      const author = $element.find('h2').text()
      const price = $element.find('.m-product-tile__price').text()
      const image = ($element
        .find('source')
        .attr('srcset') || '')
        .split(' ')[0]
        .trim()
      return {
        href: `${feedDetails.brooks.url}${href}`,
...
const filePath = path.join(dir, 'brooks.xml')
